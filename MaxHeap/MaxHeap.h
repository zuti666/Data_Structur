...
//如果子女的值大于父结点的值，则关键码大的上浮
//继续向下层比较，这样将一个集合局部调整为最大堆
	int i = start, j = 2 * i + 1;	//j是i的右子女位置
	E temp = heap[i];
	while (j <= m) {//检查是否到最后位置
		if (j<m && heap[j]<heap[j + 1])j++;//让j指向两子女中较大的
		if (temp >= heap[j])break; //大则不必调整
		else { heap[i] = heap[j]; i = j; j = 2 * j + 1; }//否则大者上移，i,j下降
	}
...
//如果子女的值大于父结点的值，则交换，这样将集合重新调整为最大堆
